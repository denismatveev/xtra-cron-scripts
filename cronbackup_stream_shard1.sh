...
######################
##### Settings ######################
BACKUPDIR="/mnt/backup/mysqlshard1"
THREADS=4
NUMBEROFFULLBACKUPS=1
NUMBERINCRBACKUPS=8 #+1 
LOCKFILE="/var/run/cronbackupshard1.lock"
MY="/var/lib/mysql1"
NOW=$(date +%Y%m%d-%H%M)
TMP="/tmp"
SERVER="bender.navixy.com" #master server
OPTIONS="--defaults-group=mysqld1"
####################################
#### lockfile ####
if [ -e "$LOCKFILE" ]
	then
...
### backup ibdata ###
    ...
    cd "$TARGET" && ssh "$SERVER" xtrabackup_55 --backup  --datadir="$MY" "$OPTIONS" --target-dir=/tmp --tmpdir=/tmp --stream=xbstream --suspend-at-end | xbstream -x & # background process
...
#### backup the rest files    
   ...
   cd "$TARGET" && ssh "$SERVER" xtrabackup_55 --backup --incremental-lsn="$LAST_LSN" "$OPTIONS" --target-dir=/tmp --tmpdir=/tmp --datadir="$MY" --stream=xbstream --suspend-at-end | xbstream -x &
    wait_suspend_file;
    echo `date +%Y%m%d-%H%M:`;echo -e "All done.OK\n"
